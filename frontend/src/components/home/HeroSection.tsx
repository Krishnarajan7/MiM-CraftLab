...
import { ArrowRight, Play, Star, Gift, Zap, Award } from "lucide-react";
        ...
        <div className="py-12 md:py-20 lg:py-28">
          <div className="grid lg:grid-cols-2 gap-10 lg:gap-16 items-center">
            ...
            >
              ...
              >
                <span className="block text-base md:text-lg text-muted-foreground font-normal mb-2">
                ...
                <span className="relative inline-block mt-2">
                  <span className="font-brand text-primary text-4xl md:text-5xl lg:text-6xl">Home</span>
                  <motion.div 
                    initial={{ scaleX: 0 }}
                    animate={{ scaleX: 1 }}
              ...
              <motion.div
                ...
                className="flex items-center gap-6 md:gap-8 mt-8 py-5 border-y border-border/40"
              >
                <div>
                ...
                <div className="flex items-center gap-2">
                  ...
                  <div>
                    <p className="text-lg md:text-xl font-bold text-foreground">4.9</p>
                    <p className="text-xs text-muted-foreground">Avg Rating</p>
                  </div>
                </div>
                <div className="w-px h-10 bg-border/60 hidden sm:block" />
                <div className="hidden sm:block">
                  <p className="text-2xl md:text-3xl font-bold text-foreground">UK</p>
                  <p className="text-xs text-muted-foreground mt-0.5">Made & Shipped</p>
                </div>
              </motion.div>
              ...
              >
                <Button asChild size="lg" className="rounded-full px-7 h-12 text-sm shadow-lg shadow-primary/20 group">
                  <Link to="/shop">
                    Browse Collection
                    <ArrowRight className="ml-2 h-4 w-4 group-hover:translate-x-0.5 transition-transform duration-200" />
                  </Link>
                </Button>
                <Button asChild variant="outline" size="lg" className="rounded-full px-5 h-12 text-sm group border-border/60">
                  <Link to="/courses" className="flex items-center gap-2">
                    <span className="w-8 h-8 rounded-full bg-primary/10 flex items-center justify-center group-hover:bg-primary/15 transition-colors duration-200">
            ...
            >
              ...
              <div className="relative rounded-2xl lg:rounded-3xl overflow-hidden shadow-2xl shadow-foreground/8">
                <div className="aspect-[4/3] lg:aspect-[5/4]">
                  <img
                    src={heroImage}
                    alt="Beautiful 3D printed products on a desk"
              ...
              {/* Floating card - Personalised */}
              <motion.div
                initial={{ opacity: 0, y: 20 }}
                animate={{ opacity: 1, y: 0 }}
                transition={{ duration: 0.5, delay: 0.5, ease: "easeOut" }}
                className="absolute -bottom-4 -left-4 lg:-bottom-6 lg:-left-8 bg-background rounded-xl lg:rounded-2xl p-4 shadow-xl border border-border/40"
              >
                <div className="flex items-center gap-3">
                  ...
                  <div>
                    <p className="font-semibold text-foreground text-sm">Personalised Gifts</p>
                    <p className="text-xs text-muted-foreground">Custom text & designs</p>
                  </div>
                </div>
              </motion.div>
              ...
              <motion.div
                ...
                className="absolute -top-3 -right-3 lg:-top-4 lg:-right-4 bg-primary text-primary-foreground rounded-full px-4 py-2 shadow-lg flex items-center gap-1.5"
              >
